...
static void DepthFirstSearchFindCycles(Ref(CPackageInfoBTree) packAll, Ptr(CPackageInfo) node)
{
	...
	while ( it )
	{
		...
		itF = packAll.FindSorted(&packInfo, PackageInfoSearchAndSortFunc);
		if ( itF && (*itF) && (PackageInfoSearchAndSortFunc(*itF, &packInfo) == 0) )
	...
	CDataVectorT<CStringBuffer>& psSearchPackage, CDataVectorT<mbchar>& psSearchKey, WUInt flags, WInt iVerbose)
{
	...
	if ( psSearchPackage.Count() > 0 )
	{
		...
		while ( it )
		{
			...
			{
				itP = packAll.FindSorted(&packInfo, PackageInfoSearchAndSortFunc);
				if ( itP && (*itP) && (PackageInfoSearchAndSortFunc(&packInfo, *itP) == 0) )
				{
					pInfo = *itP;
