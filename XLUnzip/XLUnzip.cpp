...
******************************************************************************/
// XLUnzip.cpp : Definiert den Einstiegspunkt für die Konsolenanwendung.
//
...
class XLUnzipApplication: public CApplication
{
public:
	XLUnzipApplication():
		CApplication(CStringBuffer(__FILE__LINE__ _T("XLUnzip"))),
	    m_help(false),
		m_bArchive(false),
		m_sArchive(),
		m_bOutfile(false),
		m_sOutfile()
	{
        COptionCallback<XLUnzipApplication> cb(this,&XLUnzipApplication::handleHelp);
	...
	virtual int main()
	{
		...
		XLUnzipExtractFile(m_sArchive, m_sOutfile);
