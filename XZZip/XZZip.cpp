...
******************************************************************************/
// XLZip.cpp : Definiert den Einstiegspunkt für die Konsolenanwendung.
//
...
class XZZipApplication : public CApplication
{
public:
	XZZipApplication() :
		CApplication(CStringBuffer(__FILE__LINE__ _T("XZZip"))),
		m_help(false),
		m_bArchive(false),
		m_sArchive(),
		m_bInputFile(false),
		m_sInputFile()
	{
		COptionCallback<XZZipApplication> cb(this, &XZZipApplication::handleHelp);
	...
	virtual int main()
	{
		...
		XZZipCompressFile(m_sInputFile, m_sArchive);
