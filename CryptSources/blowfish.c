...
static const dword ORIG_P[16 + 2] = {
...
static const dword ORIG_S[4][256] = {
...
void BlowfishSetup(BConstPointer key, int keylen, int num_rounds, SYMMETRIC_KEY *skey)
{
	dword x;
	dword y;
	dword z;
	...
	for (x = y = 0; x < 18; x++)
	{
		...
		for (z = 0; z < 4; z++)
		{
			A = (A << 8) | ((dword)key[y++] & 255);
			if (y == (dword)keylen)
				y = 0;
		}
		skey->blowfishkey.K[x] = ORIG_P[x] ^ A;
	}
	for (x = 0; x < 4; x++)
	{
		for (y = 0; y < 256; y++)
			skey->blowfishkey.S[x][y] = ORIG_S[x][y];
	}
	for (x = 0; x < 8; x++)
		B[x] = 0;
	for (x = 0; x < 18; x += 2)
	{
		BlowfishEncrypt(B, B, skey);
		LOAD32H(skey->blowfishkey.K[x], &B[0]);
		LOAD32H(skey->blowfishkey.K[x + 1], &B[4]);
	}
	for (x = 0; x < 4; x++)
	{
		for (y = 0; y < 256; y += 2)
		{
			BlowfishEncrypt(B, B, skey);
			LOAD32H(skey->blowfishkey.S[x][y], &B[0]);
			LOAD32H(skey->blowfishkey.S[x][y + 1], &B[4]);
		}
	}
	s_memset(B, 0, 8);
}
...
#ifndef __GNUC__
#define F(x) ((S1[get_byte(x,3)] + S2[get_byte(x,2)]) ^ S3[get_byte(x,1)]) + S4[get_byte(x,0)]
#else
#define F(x) ((skey->blowfishkey.S[0][get_byte(x,3)] + skey->blowfishkey.S[1][get_byte(x,2)]) ^ skey->blowfishkey.S[2][get_byte(x,1)]) + skey->blowfishkey.S[3][get_byte(x,0)]
#endif


void BlowfishEncrypt(BConstPointer pt, BPointer ct, SYMMETRIC_KEY *skey)
{
	dword L;
	dword R;
	int r;
...
#ifndef __GNUC__
	S1 = skey->blowfishkey.S[0];
	S2 = skey->blowfishkey.S[1];
	S3 = skey->blowfishkey.S[2];
	S4 = skey->blowfishkey.S[3];
#endif
	...
	for (r = 0; r < 16;) 
	{
		L ^= skey->blowfishkey.K[r++];  R ^= F(L);
		R ^= skey->blowfishkey.K[r++];  L ^= F(R);
		L ^= skey->blowfishkey.K[r++];  R ^= F(L);
		R ^= skey->blowfishkey.K[r++];  L ^= F(R);
	}
	R ^= skey->blowfishkey.K[17];
	L ^= skey->blowfishkey.K[16];
	STORE32H(R, &ct[0]);
	STORE32H(L, &ct[4]);
}

void BlowfishDecrypt(BConstPointer ct, BPointer pt, SYMMETRIC_KEY *skey)
{
	dword L;
	dword R;
	int r;
	...
	for (r = 15; r > 0;) 
	{
		L ^= F(R); R ^= skey->blowfishkey.K[r--];
		R ^= F(L); L ^= skey->blowfishkey.K[r--];
		L ^= F(R); R ^= skey->blowfishkey.K[r--];
		R ^= F(L); L ^= skey->blowfishkey.K[r--];
	}
	STORE32H(L, &pt[0]);
...
void BlowfishFinish(SYMMETRIC_KEY *skey)
{
}
