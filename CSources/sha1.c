...
void sha1_update(SHA1_CTX* context, const byte* data, const dword len)
{
	dword i, j;
...
void sha1_finish(SHA1_CTX* context, byte digest[SHA1_DIGEST_SIZE])
{
	dword i;
	byte  finalcount[8];
...
void digest_to_hex(const byte digest[SHA1_DIGEST_SIZE], char *output)
{
	int i, j;
	char *c = output;
...
int main(int argc, char** argv)
{
	...
	for (k = 0; k < 2; k++){
		SHA1_Init(&context);
		SHA1_Update(&context, (byte*)test_data[k], strlen(test_data[k]));
		SHA1_Final(&context, digest);
