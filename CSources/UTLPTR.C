...
sword __stdcall 
v_binsert( Array table, ConstPointer ptr, WPointer max, TSearchAndSortFunc func )
	{
	sword ix = v_bsearch( table, ptr, *max, func, UTLPTR_INSERTMODE );
	return v_insert( table, ix, ptr, max );
	}
...
sword __stdcall 
v_linsert( Array table, ConstPointer ptr, WPointer max, TSearchAndSortFunc func )
	{
	sword ix = v_lsearch( table, ptr, *max, func, UTLPTR_INSERTMODE );
	return v_insert( table, ix, ptr, max );
	}
...
sword __stdcall 
v_insert( Array table, sword ix, ConstPointer ptr, WPointer max )
	{
	word len;
	Handle tabP;
	if ( PtrCheck(table) || PtrCheck(max) || ( ix < -1 ) || ( ix >= DerefSWPointer( max ) ) )
		return -1;
	tabP = Cast( Handle, _fl_ptradd( table, ++ix * szPointer ) );
...
sword __stdcall 
v_ninsert( Array table, sword ix, Array ptr, word cnt, WPointer max )
	{
	word len;
	Handle tabP;
	if ( PtrCheck(table) || PtrCheck(max) || ( cnt == 0 ) || ( ix < -1 ) || ( ix >= DerefSWPointer( max ) ) )
		return -1L;
	tabP = Cast( Handle, _fl_ptradd( table, ++ix * szPointer ) );
	if ( 0 < (len = DerefWPointer( max ) - ix) )
		s_memmove_s( _fl_ptradd( tabP, cnt * szPointer ), len * szPointer, tabP, len * szPointer );
	DerefWPointer( max ) += cnt;
	if ( PtrCheck(ptr) )
		s_memset( tabP, 0, cnt * szPointer );
	else
		s_memmove_s( tabP, cnt * szPointer, ptr, cnt * szPointer );
...
sword __stdcall 
v_bdelete( Array table, ConstPointer ptr, WPointer max, TSearchAndSortFunc func )
	{
	sword ix = v_bsearch( table, ptr, *max, func, UTLPTR_MATCHMODE );
	return v_delete( table, ix, max );
	}
...
sword __stdcall 
v_ldelete( Array table, ConstPointer ptr, WPointer max, TSearchAndSortFunc func )
	{
	sword ix = v_lsearch( table, ptr, *max, func, UTLPTR_MATCHMODE );
	return v_delete( table, ix, max );
	}
...
sword __stdcall 
v_delete( Array table, sword ix, WPointer max )
	{
	word len;
	Array tabP;
	if ( PtrCheck(table) || PtrCheck(max) || ( ix < 0 ) || ( ix >= DerefSWPointer( max ) ) )
		return -1;
	tabP = Cast( Handle, _fl_ptradd( table, ix * szPointer ) );
	DerefWPointer( max )--;
	if ( 0 < (len = ( DerefWPointer( max ) - ix )) )
...
sword __stdcall 
v_ndelete( Array table, sword ix, word cnt, WPointer max )
	{
	word len;
	Array tabP;
	if ( PtrCheck(table) || PtrCheck(max) || ( cnt == 0 ) || ( ix < 0 ) || ( ix >= (DerefSWPointer( max ) - cnt + 1) ) )
		return -1;
	tabP = Cast( Handle, _fl_ptradd( table, ix * szPointer ) );
	DerefWPointer( max ) -= cnt;
	if ( 0 < (len = ( DerefWPointer( max ) - ix )) )
...
v_bsearch( Array table, ConstPointer ptr, word max, TSearchAndSortFunc func, sword _mode )
	...
	while ( ox >= ux )
		{
		ix = ( ( ox + ux ) / 2 ) - 1;
		erg = func( table[ix], CastMutable(Pointer, ptr) );
		if ( erg < 0 )
			ux = ix + 2;
		else if ( erg > 0 )
			ox = ix;
			...
			max--;
			for ( ; ix < max; ix++ )
				if ( ( erg = func( table[ix + 1], CastMutable(Pointer, ptr) ) ) != 0 )
					return ix;
			return max;
		} else
			{
	...
	if ( ix >= 0 )
		...
		switch ( _mode )
			{
		case UTLPTR_INSERTMODE:
			if ( func( table[ix], CastMutable(Pointer, ptr) ) > 0 )
				ix--;
			break;
		case UTLPTR_SEARCHMODE:
			if ( func( table[ix], CastMutable(Pointer, ptr) ) >= 0 )
...
sword __stdcall 
v_lsearch( Array table, ConstPointer ptr, word max, TSearchAndSortFunc func, sword _mode )
	{
	sword ix = max;
	Array pt = table;
	sword res;
	...
	if ( _mode == UTLPTR_INSERTMODE )
		return max - 1;
	for ( ; ix > 0; ix--, pt = CastAny(Array,l_ptradd( pt, szPointer ) ) )
		{
		res = func( *pt, ptr );
		if ( res == 0 )
			return max - ix;
...
void __stdcall 
v_heapsort( Array _heap, word cnt, TSearchAndSortFunc _cmp )
	{
	word min;
	word ix;
	word i2;
...
_v_quicksort(Array _heap, sword l, sword r, TSearchAndSortFunc _cmp)
{
	...
	_v_quicksort(_heap, l, j, _cmp);
	_v_quicksort(_heap, i, r, _cmp);
}

void __stdcall 
v_quicksort( Array _heap, word cnt, TSearchAndSortFunc _cmp )
{
	_v_quicksort(_heap, 0, cnt - 1, _cmp);
}
...
_v_uquicksort(Array _heap, sword l, sword r, Pointer user, TSearchAndSortUserFunc _cmp)
{
	...
	_v_uquicksort(_heap, l, j, user, _cmp);
	_v_uquicksort(_heap, i, r, user, _cmp);
}
...
sword __stdcall 
s_binsert( Pointer table, dword size, ConstPointer ptr, WPointer max, TSearchAndSortFunc func )
	{
	sword ix = s_bsearch( table, size, ptr, *max, func, UTLPTR_INSERTMODE );
	return s_insert( table, size, ix, ptr, max );
	}
...
sword __stdcall 
s_linsert( Pointer table, dword size, ConstPointer ptr, WPointer max, TSearchAndSortFunc func )
	{
	sword ix = s_lsearch( table, size, ptr, *max, func, UTLPTR_INSERTMODE );
	return s_insert( table, size, ix, ptr, max );
	}
...
sword __stdcall 
s_ninsert( Pointer table, dword size, sword ix, ConstPointer ptr, word cnt, WPointer max )
	{
	word len;
	Pointer tabP;
	if ( PtrCheck(table) || PtrCheck(max) || (size == 0) || (cnt == 0) || ( ix < -1 ) || ( ix >= DerefSWPointer( max ) ) )
		return -1;
	tabP = l_ptradd( table, (++ix) * size );
	if ( 0 < (len = DerefWPointer( max ) - ix) )
		s_memmove_s( _fl_ptradd( tabP, cnt * size ), len * size, tabP, len * size );
...
sword __stdcall 
s_bdelete( Pointer table, dword size, ConstPointer ptr, WPointer max, TSearchAndSortFunc func )
	{
	sword ix = s_bsearch( table, size, ptr, *max, func, UTLPTR_MATCHMODE );
...
sword __stdcall 
s_ndelete( Pointer table, dword size, sword ix, word cnt, WPointer max )
	{
	word len;
	Pointer tabP;
	if ( PtrCheck(table) || PtrCheck(max) || (size == 0) || (cnt == 0) || ( ix < 0 ) || ( ix >= (DerefSWPointer( max ) - cnt + 1) ) )
		return -1;
	tabP = l_ptradd( table, ix * size );
	DerefWPointer( max ) -= cnt;
	if ( (len = DerefWPointer( max ) - ix) != 0 )
		s_memmove_s( tabP, len * size, _fl_ptradd( tabP, cnt * size ), len * size );
	s_memset( _fl_ptradd( tabP, len * size ), 0, cnt * size );
	if ( ix >= DerefSWPointer( max ) )
		return ix - cnt;
...
void __stdcall
s_heapsort( Pointer _heap, dword _size, word cnt, TSearchAndSortFunc _cmp )
	{
	word min;
	word ix;
	word i2;
	...
	for ( iix = cnt / 2; iix > 0; iix-- )
		...
		while ( i <= ( cnt / 2 ) )
			{
			ix = ( i - 1 ) * Castword(_size);
			i2 = ( ix << 1 ) + Castword(_size);
			min = (_cmp( l_ptradd( _heap, ix ), l_ptradd( _heap, i2 ) ) <= 0) ? ix : i2;
			if ( ( ( i << 1 ) + 1 ) <= cnt )
				{
				i21 = i2 + Castword(_size);
				if ( _cmp( l_ptradd( _heap, i21 ), l_ptradd( _heap, min ) ) <= 0 )
					min = i21;
				}
			if ( min == ix )
				break;
			v_swapbytes( l_ptradd( _heap, ix ), l_ptradd( _heap, min ), Castword(_size) );
			i = ( min + Castword(_size) ) / Castword(_size);
			}
		}
	for ( iix = cnt - 1, sz1 = sz = ( cnt - 1 ) * Castword(_size); iix > 0; iix--, sz -= Castword(_size) )
		{
		v_swapbytes( _heap, l_ptradd( _heap, sz ), Castword(_size) );
		/* reheap( iix, 1 ); */
		i = 1;
		while ( i <= ( iix / 2 ) )
			{
	...
	for ( iix = cnt / 2, sz = 0 /* , sz1 = (cnt-1) * Castword(_size) */ ; iix > 0; iix--, sz += Castword(_size), sz1 -= Castword(_size) )
		v_swapbytes( l_ptradd( _heap, sz ), l_ptradd( _heap, sz1 ), Castword(_size) );
	}
...
s_uheapsort( Pointer _heap, dword _size, word cnt, TSearchAndSortUserFunc _cmp, Pointer user )
	...
	for ( iix = cnt / 2; iix > 0; iix-- )
		...
		while ( i <= ( cnt / 2 ) )
			...
			ix = ( i - 1 ) * Castword(_size);
			i2 = ( ix << 1 ) + Castword(_size);
			min = (_cmp( l_ptradd( _heap, ix ), l_ptradd( _heap, i2 ), user ) <= 0) ? ix : i2;
			if ( ( ( i << 1 ) + 1 ) <= cnt )
				{
				i21 = i2 + Castword(_size);
				if ( _cmp( l_ptradd( _heap, i21 ), l_ptradd( _heap, min ), user ) <= 0 )
					min = i21;
				}
			if ( min == ix )
				break;
...
sword __stdcall 
v_searchptr( Array table, Pointer ptr, word max )
	{
	sword ix = max;
	if ( PtrCheck(table) )
		return -1;
	for ( ; ix > 0; ix--, table = CastAny(Array,l_ptradd( table, szPointer )) )
...
int strcmp2( CConstPointer s1, CConstPointer s2 ) {
	return s_strcmp(CastMutable(CPointer,s1),CastMutable(CPointer,s2));
}

sword 
_cmp( ConstPointer p1, ConstPointer p2 )
	{
	return strcmp2( ( CPointer ) p1, ( CPointer ) p2 );
	}

sword 
_rcmp( ConstPointer p1, ConstPointer p2 )
	{
	return strcmp2( ( CPointer ) p2, ( CPointer ) p1 );
	}
...
_tmain( int argc, CPointer *argv )
	...
	_RPTF0( _CRT_ASSERT, "** Run 1: test all equal **" );
	for ( ix = 0; ix < 10; ix++ )
		{
		jx = v_bsearch( CastAny(Array,test), test[ix], 10, _cmp, UTLPTR_MATCHMODE );
		_ASSERTE(jx == ix);
		jx = v_bsearch( CastAny(Array,test), test[ix], 10, _cmp, UTLPTR_INSERTMODE );
		_ASSERTE(jx == ix);
		jx = v_bsearch( CastAny(Array,test), test[ix], 10, _cmp, UTLPTR_SEARCHMODE );
		_ASSERTE(jx == ix);
		}
	_RPTF0( _CRT_ASSERT, "** Run 1b: test all equal (rcmp) **" );
	for ( ix = 0; ix < 10; ix++ )
		{
		jx = v_bsearch( CastAny(Array,rtest), rtest[ix], 10, _rcmp, UTLPTR_MATCHMODE );
		_ASSERTE(jx == ix);
		jx = v_bsearch( CastAny(Array,rtest), rtest[ix], 10, _rcmp, UTLPTR_INSERTMODE );
		_ASSERTE(jx == ix);
		jx = v_bsearch( CastAny(Array,rtest), rtest[ix], 10, _rcmp, UTLPTR_SEARCHMODE );
		_ASSERTE(jx == ix);
		}
	_RPTF0( _CRT_ASSERT, "** Run 2: test all unequal **" );
	for ( ix = 1; ix < 12; ix++ )
		{
		jx = v_bsearch( CastAny(Array,test), test2[ix], 10, _cmp, UTLPTR_MATCHMODE );
		_ASSERTE(jx == -1);
		jx = v_bsearch( CastAny(Array,test), test2[ix], 10, _cmp, UTLPTR_INSERTMODE );
		_ASSERTE( (!(( jx > -1 ) && ( strcmp2( test2[ix], test[jx] ) < 0 )))
				&& (!(( jx < 9 ) && ( strcmp2( test2[ix], test[jx + 1] ) > 0 )))
				&& (!(jx != (ix - 2)))
			 );
		jx = v_bsearch( CastAny(Array,test), test2[ix], 10, _cmp, UTLPTR_SEARCHMODE );
		_ASSERTE( (!(( jx > 0 ) && ( strcmp2( test2[ix], test[jx - 1] ) < 0 )))
				&& (!(( jx < 9 ) && ( strcmp2( test2[ix], test[jx] ) > 0 )))
				&& (!(jx != Min((ix - 1),9)))
			 );
		}
	_RPTF0( _CRT_ASSERT, "** Run 2b: test all unequal (rcmp) **" );
	for ( ix = 2; ix < 13; ix++ )
		{
		jx = v_bsearch( CastAny(Array,rtest), rtest2[ix], 10, _rcmp, UTLPTR_MATCHMODE );
		_ASSERTE( jx == -1 );
		jx = v_bsearch( CastAny(Array,rtest), rtest2[ix], 10, _rcmp, UTLPTR_INSERTMODE );
		_ASSERTE ( (!(( jx > -1 ) && ( strcmp2( rtest2[ix], rtest[jx] ) > 0 )))
				&& (!(( jx < 9 ) && ( strcmp2( rtest2[ix], rtest[jx + 1] ) < 0 )))
				&& (!(jx != (ix - 3)))
			 );
		jx = v_bsearch( CastAny(Array,rtest), rtest2[ix], 10, _rcmp, UTLPTR_SEARCHMODE );
		_ASSERTE ( (!(( jx > 0 ) && ( strcmp2( rtest2[ix], rtest[jx - 1] ) > 0 )))
				&& (!(( jx < 9 ) && ( strcmp2( rtest2[ix], rtest[jx] ) < 0 )))
				&& (!(jx != Min((ix - 2),9)))
			 );
		}
	_RPTF0( _CRT_ASSERT, "** Run 3: test all equal with identicals **" );
	for ( ix = 1; ix < 12; ix++ )
		{
		jx = v_bsearch( CastAny(Array,test3), test2[ix], 34, _cmp, UTLPTR_MATCHMODE );
		_ASSERTE ( jx == ( 3 * (ix - 1) ) );
		jx = v_bsearch( CastAny(Array,test3), test2[ix], 34, _cmp, UTLPTR_INSERTMODE );
		_ASSERTE ( jx == (( 3 * (ix - 1) ) + 1) );
		jx = v_bsearch( CastAny(Array,test3), test2[ix], 34, _cmp, UTLPTR_SEARCHMODE );
		_ASSERTE ( jx == ( 3 * (ix - 1) ) );
		}
	_RPTF0( _CRT_ASSERT, "** Run 3b: test all equal with identicals (rcmp) **" );
	for ( ix = 2; ix < 13; ix++ )
		{
		jx = v_bsearch( CastAny(Array,rtest3), rtest2[ix], 34, _rcmp, UTLPTR_MATCHMODE );
		_ASSERTE ( jx == (( 3 * (ix - 1) ) - 1) );
		jx = v_bsearch( CastAny(Array,rtest3), rtest2[ix], 34, _rcmp, UTLPTR_INSERTMODE );
		_ASSERTE ( jx == ( 3 * (ix - 1) ) );
		jx = v_bsearch( CastAny(Array,rtest3), rtest2[ix], 34, _rcmp, UTLPTR_SEARCHMODE );
		_ASSERTE ( jx == (( 3 * (ix - 1) ) - 1) );
		}
	_RPTF0( _CRT_ASSERT, "** Run 4: test v_insert/v_delete, v_ninsert/v_ndelete **" );
	for ( ix = 0; ix < 128; ix++ )
		buf1[ix] = NULL;
	for ( ix = 0; ix < 64; ix++ )
		buf1[ix] = ( Pointer ) ( 0xFFFF0000 + ix );
	max1 = 64;
	for ( ix = 0; ix < 128; ix += 2 )
		v_insert( buf1, ix, ( Pointer ) ( 0xFFFF0000 + 128 ), &max1 );
	for ( ix = 0; ix < 128; ix++ )
		{
		if ( ix % 2 )
			_ASSERTE ( buf1[ix] == ( Pointer ) ( 0xFFFF0000 + 128 ) );
		else
			_ASSERTE ( buf1[ix] == ( Pointer ) ( 0xFFFF0000 + ( ix / 2 ) ) );
		}
	for ( ix = 1; ix < max1; ix++ )
		v_delete( buf1, ix, &max1 );
	_ASSERTE( max1 == 64 );
	for ( ix = 0; ix < 64; ix++ )
		{
		_ASSERTE ( buf1[ix] == ( Pointer ) ( 0xFFFF0000 + ix ) );
		}
	for ( ix = 0; ix < 128; ix++ )
		buf1[ix] = NULL;
	for ( ix = 0; ix < 32; ix++ )
		buf1[ix] = ( Pointer ) ( 0xFFFF0000 + ix );
	for ( ix = 0; ix < 3; ix++ )
		p1[ix] = ( Pointer ) ( 0xFFFF0000 + 128 );
	max1 = 32;
	for ( ix = 0; ix < 128; ix += 4 )
		v_ninsert( buf1, ix, CastAnyPtr(Pointer, &p1), 3, &max1 );
	for ( ix = 0; ix < 128; ix++ )
		{
		if ( ix % 4 )
			_ASSERTE ( buf1[ix] == ( Pointer ) ( 0xFFFF0000 + 128 ) );
		else
			_ASSERTE ( buf1[ix] == ( Pointer ) ( 0xFFFF0000 + ( ix / 4 ) ) );
		}
	for ( ix = 1; ix < max1; ix++ )
		v_ndelete( buf1, ix, 3, &max1 );
	_ASSERTE ( max1 >= 32 );
	for ( ix = 0; ix < 32; ix++ )
		{
		_ASSERTE ( buf1[ix] == ( Pointer ) ( 0xFFFF0000 + ix ) );
		}
	_RPTF0( _CRT_ASSERT, "** Run 5: test s_insert/s_delete, s_ninsert/s_ndelete **" );
	for ( ix = 0; ix < 256; ix++ )
		buf2[ix] = 0;
	for ( ix = 0; ix < 128; ix++ )
		buf2[ix] = ( mbchar ) ix;
	max2 = 128;
	jx = 128;
	for ( ix = 0; ix < 256; ix += 2 )
		s_insert( buf2, szchar, ix, &jx, &max2 );
	for ( ix = 0; ix < 256; ix++ )
		{
		if ( ix % 2 )
			_ASSERTE ( buf2[ix] == ( mbchar ) 128 );
		else
			_ASSERTE ( buf2[ix] == ( mbchar ) ( ix / 2 ) );
		}
	for ( ix = 1; ix < 129; ix++ )
		s_delete( buf2, szchar, ix, &max2 );
	_ASSERTE ( max2 == 128 );
	for ( ix = 0; ix < 128; ix++ )
		{
		_ASSERTE ( buf2[ix] == ( mbchar ) ix );
		}
	for ( ix = 0; ix < 256; ix++ )
		buf2[ix] = 0;
	for ( ix = 0; ix < 64; ix++ )
	...
	for ( ix = 0; ix < 256; ix += 4 )
		s_ninsert( buf2, szchar, ix, p2, 3, &max2 );
	for ( ix = 0; ix < 256; ix++ )
		{
		if ( ix % 4 )
			_ASSERTE ( buf2[ix] == ( mbchar ) 128 );
		else
			_ASSERTE ( buf2[ix] == ( mbchar ) ( ix / 4 ) );
		}
	for ( ix = 1; ix < max2; ix++ )
		s_ndelete( buf2, szchar, ix, 3, &max2 );
	_ASSERTE ( max2 == 64 );
	for ( ix = 0; ix < 64; ix++ )
		{
		_ASSERTE( buf2[ix] == ( mbchar ) ix );
		}
	_RPTF0( _CRT_ASSERT, "** Completed **" );
	return 0;
	}
#endif
/* end of UTLPTR.H */
