...
static int testValue(OSVERSIONINFOEX * value, DWORD verPart, DWORDLONG eq, DWORDLONG gt)
{
...
static DWORDLONG gtFor(DWORD target)
{
	return VerSetConditionMask(0, target, VER_GREATER);
}

static DWORDLONG eqFor(DWORD target)
{
	return VerSetConditionMask(0, target, VER_EQUAL);
}

#define findPartTemplate(T) \
static BOOL findPart##T(T * part, DWORD partType, OSVERSIONINFOEX * ret, T a, T b) \
{ \
	int funx = FV_EQUAL; \
	\
